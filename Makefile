...
FUZZ_TARGETS ?= parser type_expr lowering_stub checker
...
ci: fmt-check lint test-fast test-cli-verification test-downstream-checkers flagship-smoke examples-smoke roadmap-demo-smoke stdlib-baseline-check conformance-check diagnostic-coverage-check repo-contracts bench-check package-check sdist-smoke
