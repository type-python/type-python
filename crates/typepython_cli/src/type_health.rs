...
#[cfg(test)]
mod type_lock_string_tests {
    use super::toml_string;

    #[test]
    fn toml_strings_round_trip_untrusted_text() {
        for value in [
            "plain",
            "quote \" and slash \\",
            "first line\nsecond line\r\nthird\tcolumn",
            "controls \u{0000}\u{0008}\u{000c}\u{001f}\u{007f}",
        ] {
            let document = format!("value = {}\n", toml_string(value));
            let parsed: toml::Value =
                toml::from_str(&document).expect("rendered string should be valid TOML");
