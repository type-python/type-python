...
fn check_rejects_tainted_module_member_call_argument_when_taint_is_enabled() {
    ...
    let rendered = result.diagnostics.as_text();
    assert!(rendered.contains("TPY4001"), "{rendered}");
    assert!(rendered.contains("call to `lib.consume`"), "{rendered}");
    assert!(rendered.contains("Tainted[str, \"html\"]"), "{rendered}");
}
...
#[test]
fn check_uses_imported_method_taint_source_sink_and_sanitizer_facts() {
    let root = create_temp_typepython_root();
    let lib_path = root.join("lib.tpy");
    let app_path = root.join("app.tpy");
    let lib_source = concat!(
    ...
    let app_source = concat!(
        "from lib import Html as SafeHtml\n",
        "from lib import Request as Req\n",
        "from lib import Response as Resp\n\n",
        "def unsafe() -> None:\n",
        "    raw = Req().body()\n",
        "    Resp().render(raw)\n\n",
        "def safe() -> None:\n",
        "    raw = Req().body()\n",
        "    Resp().render(SafeHtml().escape(raw))\n",
    );
    fs::write(&lib_path, lib_source).expect("temp source should be written");
    fs::write(&app_path, app_source).expect("temp source should be written");
    ...
    let rendered = result.diagnostics.as_text();
    assert_eq!(rendered.matches("TPY4028").count(), 1, "{rendered}");
    assert!(rendered.contains("tainted source result flows into sink `Resp.render`"), "{rendered}");
