...
#[expect(
    clippy::too_many_arguments,
    reason = "type id expression resolution mirrors the direct expression metadata shape"
)]
#[allow(dead_code, reason = "type id entrypoints are staged migration helpers")]
pub(super) fn resolve_direct_expression_type_id(
    store: &mut TypeStore,
    node: &typepython_graph::ModuleNode,
    nodes: &[typepython_graph::ModuleNode],
    signature: Option<&str>,
