...
fn hover_renders_effect_summary_for_adapter_declared_callable() {
    ...
    assert!(contents.contains("function fetch"), "{contents}");
    assert!(contents.contains("Effect summary: row [io.net, time]"), "{contents}");
}
...
fn hover_renders_inferred_stdlib_effect_summary_for_local_callable() {
    ...
    assert!(contents.contains("function now"), "{contents}");
    assert!(contents.contains("Effect summary: inferred row [time]"), "{contents}");
}
...
fn signature_help_selects_overload_by_active_parameter() {
    let config = temp_config(
        ...
        "overload def target(a: int) -> int: ...\n\
         overload def target(a: int, b: str) -> int: ...\n\
         def target(a: int, b: str = \"x\") -> int:\n    return a\n\n\
         value = target(1, )\n",
    );
    let mut server = Server::new(config.clone());
