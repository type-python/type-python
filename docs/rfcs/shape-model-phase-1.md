...
Project builds and LSP preview emit gate non-`TypedDict` projection materialization behind `[experimental].accepted_features = ["shape_transforms"]` plus `[experimental].shape_transforms = true`. With the gate disabled, `TypedDict` projections remain available while class-backed projection aliases fail closed instead of silently changing the public surface.
